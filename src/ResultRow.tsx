...
const logos:{[keys:string]:Logo} = {
    ...
    moonpay: {source:'https://www.moonpay.com/assets/logo-full-purple.svg'}
};
export default function ResultRow({loading, providerName, btc}:ResultRowProps){
    return(
        ...
        bg-gradient-to-r from-purple-500/10 to-blue-500/10 p-4 my-2
        overflow-hidden">
                ...
                {providerName && (
                    <div className="grow items-center flex">
                        <img 
                            ...
                            className={
                                "h-7"+(logos[providerName]?.invert ? 'invert':'')
                            }
                            alt="" />
                    </div>
                )}
            ...
            {loading && (
                ...
                via-red-900/50 to-transparent skeleton-animation border-b border-t border-white/25"/>
            )}
        </a>
    )
}